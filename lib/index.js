...
function plugin(options) {
  ...
  return function(files, metalsmith, done){
    ...
    Object.keys(files).forEach(function(file) {
      var directory = dirname(file).toLowerCase(),
          directories = directory.split(sep);

      if (options.directories && options.directories.indexOf(directories[0]) == -1) {
        //debug("Skipping", file, "since it's not in one of the specified directories");
        return;
      }
      ...
      */ 
      ...
      debug("path", path, linkset.pattern);
      var fam = family(file, files);
