  ...
  return (files, metalsmith, done) => {
    ...
    Object.keys(files).forEach(file => {
      ...
      if (typeof options.ignoreFilter === "object") {
        if (!Array.isArray(options.ignoreFilter)) {
          options.ignoreFilter = [ options.ignoreFilter ];
        }

        options.ignoreFilter.forEach(f => {
          inDirectories = !inDirectories ? false : !f.test(data.path);
        });
      }
    ...
    debug(metadata.collections.notes);
  };
};
